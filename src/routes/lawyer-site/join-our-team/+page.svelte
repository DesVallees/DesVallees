	...
	<section>
		...
		<div class="testimonialCards">
			<Testimonial
				...
				name="Emily Davis, {$dictionary.familyLawAttorney}"
				location="Baltimore, MD"
				text={$dictionary.joiningLegalCollab}
			/>
		</div>
	</section>
		...
		<form>
			...
			{#if resumeName}
				<div class="resume" transition:slide={{ duration: 100 }}>
					...
					<button
						class="deleteResume baseButton"
						on:click={deleteResume}
						aria-label={$dictionary.deleteResume}
						type="button"><ion-icon name="close-circle-outline" /></button
					>
				</div>
			{/if}
