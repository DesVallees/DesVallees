...
<nav class="primaryNav">
	...
	<section in:fade|global>
		<div>
			<a
				on:click|preventDefault={() => linkBehaviour('/intranet')}
				href="/intranet"
				class="headerLink baseButton {$page.url.pathname === '/intranet' ? 'active' : ''}"
			>
				<span class="headerLinkText">{$dictionary.home}</span>
			</a>
			<a
				on:click|preventDefault={() => linkBehaviour('/intranet/people')}
				href="/intranet/people"
				class="headerLink baseButton {$page.url.pathname === '/intranet/people'
					? 'active'
					: ''}"
			>
			...
			<a
				on:click|preventDefault={() => linkBehaviour('/intranet/general')}
				href="/intranet/general"
				class="headerLink baseButton {$page.url.pathname === '/intranet/general'
					? 'active'
					: ''}"
			>
		...
		<div>
			...
			<ChangeLanguage
				...
				style="font-size: 1.1rem; width: 100%; justify-content: flex-start;"
			/>
			<a
				bind:this={lastFocusableElement}
				on:click|preventDefault={() => linkBehaviour('/intranet/profile')}
				href="/intranet/profile"
				class="headerLink baseButton {$page.url.pathname === '/intranet/profile'
					? 'active'
					: ''}"
