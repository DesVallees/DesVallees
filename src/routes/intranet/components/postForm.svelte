	...
	async function handleSubmit(this: HTMLFormElement) {
		...
		if (response.status === 200) {
			if (location.pathname === '/intranet/people') {
				...
				parentCommentId &&
				location.pathname !== `/intranet/replies/${parentCommentId}`
			) {
...
<form on:submit|preventDefault={handleSubmit} action="?/createPost">
	...
	<textarea
		...
		autocomplete="off"
		name="content"
		placeholder={$dictionary.whatDoYouWantToSay}
	/>

	{#if imageSrc}
		<div style="position: relative;">
			<button class="removeImg" on:click={removeImage}
		...
		<div class="addImage">
			{#if !imageSrc}
				...
				<label
					...
					tabindex="0"
					on:keypress={(event) => {
						event.key == 'Enter' || event.key == ' ' ? addAnImage.click() : '';
					}}
					aria-label={$dictionary.addAnImage}><ion-icon name="image-outline" /></label
				>
				<input
					type="file"
					id="image"
					accept="image/*"
					style="display: none;"
					bind:this={addAnImage}
					on:change={previewImage}
				/>
			{/if}
		</div>
		<div class="rightButtons">
			<button type="button" class="link" on:click={() => dispatch('close')}
				>{$dictionary.cancel}</button
			>
			<button bind:this={lastFocusableElement} type="submit" class="link"
