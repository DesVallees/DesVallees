...
<svelte:window
	on:resize={() => (isMobileDevice = window.innerWidth <= window.innerHeight)}
...
<div class="guitars" in:fade|global>
	{#key $chosenGuitarIndex}
		<div
			class="above"
			in:fly={{
				x: isMobileDevice ? 0 : lastMove ? (lastMove === 'next' ? xDis : -xDis) : '0',
				y: isMobileDevice ? 50 : 0,
				duration: 600,
			}}
			style="height: {aboveContentHeight}px;"
		>
			<div class="empty" />
	...
	<div class="centralLine" style={isMobileDevice ? `height: ${middleContentHeight}px;` : ''}>
		<div class="centralLineContent">
			<button
				class="previous baseButton"
				on:click={previousGuitar}
				aria-label={$dictionary.guitarraAnterior}
			>
				<ion-icon name="chevron-back-outline" style="transform: translateX(-3px);" />
			</button>

			{#key $chosenGuitarIndex}
				<div
					class="guitarImage"
					bind:offsetHeight={middleContentHeight}
					in:fly={{
						x: lastMove ? (lastMove === 'next' ? xDis : -xDis) : '0',
						duration: 600,
						delay: isMobileDevice ? 0 : 100,
					}}
				>
					<GuitarImage
						fileNames={chosenGuitar.fileNames}
						bind:currentFileIndex
						name={chosenGuitar.name[$language]}
						extension={chosenGuitar.filesExtension}
					/>
				</div>
			{/key}

			<button
				class="next baseButton"
				on:click={nextGuitar}
				aria-label={$dictionary.guitarraSiguiente}
			>
	...
	{#key $chosenGuitarIndex}
		<div class="belowWrapper" style="height: {belowContentHeight}px;">
			<div
				...
				bind:offsetHeight={belowContentHeight}
			>
