...
<nav>
	...
	<section id="xnav" class="x">
		<a href="/ccs" class="link {$page.url.pathname === '/ccs' ? 'active' : ''}"
			>{$dictionary.inicio}</a
		>
		<a href="/ccs/acerca" class="link {$page.url.pathname === '/ccs/acerca' ? 'active' : ''}"
			>{$dictionary.acercaDeNosotros}</a
		>
		<a
			href="/ccs/contactanos"
			class="link {$page.url.pathname === '/ccs/contactanos' ? 'active' : ''}"
