<script lang="ts">
	...
	function findObjectByName(nameToFind: string) {
		...
		return guitarStorage.find(
			(item) => item.name.es.replace(/-/g, ' ').toLowerCase() === formattedNameToFind,
		);
	}
...
<div class="main" in:fade|global>
	...
	{#if chosenGuitar}
		...
		<div class="interactives" in:fade|global={{ delay: 600 }}>
			...
			<ShareButton
				justIcon={false}
				style="color: var(--content); padding: 10px 1.2em;"
				url={guitarSpanishName}
				title={chosenGuitar.name[$language]}
	...
	{:else}
		<Typewriter
			phrase={$dictionary.error}
			style="font-size: 2rem; width: 80%;"
			typingSpeed={50}
		/>
	{/if}
</div>
