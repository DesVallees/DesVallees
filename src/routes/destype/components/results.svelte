...
<h1 class="screenReaderResults" bind:this={screenReaderResults} tabindex="-1">
	{$dictionary.wordsPerMinute}: {getWordsPerMinute()}. {$dictionary.accuracy}: {getAccuracy()}%.
</h1>

<div in:blur|global>
	<ResultStat
		title={$dictionary.wpm}
		tooltip={$dictionary.wordsPerMinute}
		score={Math.trunc($wordsPerMinute)}
	/>
...
<div in:blur|global>
	...
	<button on:click={resetGame} class="play" aria-label={$dictionary.playAgain}
		>{$dictionary.playAgain}</button
	>
</div>
